...
export const ImageCmp = ({ theimgUrl, index }) => {
    return (
      <Image
        source={theimgUrl}
        resizeMode="contain"
...
export const People = () => {
    return (
      <View style={{flexDirection: "row" }}>
        {
            [assets.person02, assets.person03, assets.person04].map((theimgUrl, index) => (
                <ImageCmp theimgUrl={theimgUrl} index={index} key={`People-${index}`}/>
            ))
        }
      </View>
    )
